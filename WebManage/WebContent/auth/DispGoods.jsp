...
<body>
	<div class='fix_wrapper' style="margin-top:50px;margin-right:100px"> 
		<h2>商品修改</h2>
		<aside class='form'>
		<form accept-charset="UTF-8" action="${pageContext.request.contextPath}/goods.do?method=upd" method="post">
					<div class='component'>
					...
					<div class='component'>
						...
						<c:forEach items="${requestScope.list }" var="gcate">
							<option value="${gcate.id}" ${gcate.id==goods.gcate.id?"selected":""}>${gcate.name}</option>
						</c:forEach>
						</select>
					</div>
					<div class='component'>
						<label for="GoodsDescribe">说明:</label> <textarea class="text_field_280"
							id="GoodsDescribe" name="GoodsDescribe" placeholder="Enter Describe" style="resize:none" rows="8" >${goods.desc }</textarea>
					</div>
