...
<body>
		...
		<div class="smallcontain" style="border: 1px solid #ccc">
			...
			<form class="form-search"  style="margin-top: 30px">
				...
				<input type="button" style="margin-left: 250px" class="btn"
					value="添加新闻" onclick="add()" /> <input type="button"
			...
			<table class="table" contenteditable="true">
				...
				<tbody>
					<c:forEach var="news" items="${requestScope.pageModel.list}" >
						<tr>
							<td><input type="checkbox" name="singleChecked"
								style="width: 20px; height: 20px" value="${news.id}" ></td>
							<td>${news.title}</td>
							<td>${news.author}</td>
							<td><a href="#" onclick="mod(${news.id})">修改</a></td>
							<td><a href="#" onclick="del(${news.id})">删除</a></td>
							<td><a href="#" onclick="see(${news.id})">查看</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
	...
	<script type="text/javascript">
		...
		function topPage(){
			...
			window.location="${pageContext.request.contextPath}/news.do?method=seePage&pageNo=${requestScope.pageModel.getTopPageNo()}"+"&str="+str;
	 	}
		function previousPage(){
			var str = document.getElementById("str").value;
	    	window.location="${pageContext.request.contextPath}/news.do?method=seePage&pageNo=${requestScope.pageModel.getPreviousPageNo()}"+"&str="+str;
    	}
    	function nextPage(){
    		var str = document.getElementById("str").value;
	    	window.location="${pageContext.request.contextPath}/news.do?method=seePage&pageNo=${requestScope.pageModel.getNextPageNo()}"+"&str="+str;
    	}
    	function bottomPage(){
    		var str = document.getElementById("str").value;
	    	window.location="${pageContext.request.contextPath}/news.do?method=seePage&pageNo=${requestScope.pageModel.getBottomPageNo()}"+"&str="+str;
		}
    
    	function submitSearch(){
    		var str = document.getElementById("str").value;
    		window.location = "${pageContext.request.contextPath}/news.do?method=seePage&str="+str;
    	}
	    ...
	    function delPage(){
     		...
     		if(window.confirm("确认要删除吗？")){
     		  	window.location = "${pageContext.request.contextPath}/news.do?method=delPage&ids="+ids;
     		}
     	}
	    function mod(id){
	    	if(confirm('确认要修改吗')){
	    		window.location = "${pageContext.request.contextPath}/news.do?method=see&id="+id;
	    	}
	    }
	    function see(id){
    		window.location = "${pageContext.request.contextPath}/news.do?method=see&id="+id+"&flag=on";
	    }
	    function del(id){
	    	if(confirm('确认要删除吗')){
	    		window.location = "${pageContext.request.contextPath}/news.do?method=del&id="+id;
	    	}
	    }
	    function add(){
	    	window.location="${pageContext.request.contextPath}/auth/AddNews.jsp";
	    }
	</script>
</body>
</html>